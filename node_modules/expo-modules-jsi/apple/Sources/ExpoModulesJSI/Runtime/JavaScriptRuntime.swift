...
open class JavaScriptRuntime: Equatable, Identifiable, @unchecked Sendable {
  ...
  public func createHostObject(
    ...
    func getter(
      ...
      let resultAddr = unsafeBitCast(resultPtr, to: UInt.self)

      return withGuaranteedContext(context) { (context: HostObjectContext, runtime) in
        return JavaScriptActor.assumeIsolated {
          return forwardingSwiftErrorsToJS(runtime: runtime) {
            try context.get(propertyName).writeJSIValue(to: UnsafeMutablePointer<facebook.jsi.Value>(bitPattern: resultAddr)!)
          }
        }
      }
    }
...
private func createFunctionClosure(
  ...
  func call(
    ...
    // floor.
    // Pointers cannot cross the sending closure below, so pass them as UInt and rebuild inside.
    let thisAddr = unsafeBitCast(thisPtr, to: UInt.self)
    let argumentsAddr = unsafeBitCast(argumentsPtr, to: UInt.self)
    let resultAddr = unsafeBitCast(resultPtr, to: UInt.self)
    ...
    return withGuaranteedContext(context) { (context: HostFunctionContext, runtime) in
      return JavaScriptActor.assumeIsolated {
        return forwardingSwiftErrorsToJS(runtime: runtime) {
          let this = UnsafeMutablePointer(mutating: UnsafePointer<facebook.jsi.Value>(bitPattern: thisAddr)!).move()
          let arguments = JavaScriptValuesBuffer(runtime, start: UnsafePointer<facebook.jsi.Value>(bitPattern: argumentsAddr)!, count: argumentsCount)
          let thisValue = JavaScriptValue(runtime, this)
          try context.call(thisValue, consume arguments).writeJSIValue(to: UnsafeMutablePointer<facebook.jsi.Value>(bitPattern: resultAddr)!)
        }
      }
    }
  }
  ...
  func call(
    ...
    return withGuaranteedContext(context) { (context: UnownedThisHostFunctionContext, runtime) in
      return JavaScriptActor.assumeIsolated {
        return forwardingSwiftErrorsToJS(runtime: runtime) {
          let arguments = JavaScriptValuesBuffer(runtime, start: UnsafePointer<facebook.jsi.Value>(bitPattern: argumentsAddr)!, count: argumentsCount)
          let thisValue = JavaScriptUnownedValue(runtime.pointee, UnsafePointer<facebook.jsi.Value>(bitPattern: thisAddr)!)
          try context.call(thisValue, consume arguments).writeJSIValue(to: UnsafeMutablePointer<facebook.jsi.Value>(bitPattern: resultAddr)!)
        }
      }
    }
