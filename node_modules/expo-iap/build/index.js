...
export const promotedProductListenerIOS = (listener) => {
    if (Platform.OS !== 'ios') {
        ExpoIapConsole.warn('promotedProductListenerIOS: This listener is only available on iOS');
        return { remove: () => { } };
    }
...
export const userChoiceBillingListenerAndroid = (listener) => {
    if (Platform.OS !== 'android') {
        ExpoIapConsole.warn('userChoiceBillingListenerAndroid: This listener is only available on Android');
        return { remove: () => { } };
    }
...
export const getAvailablePurchases = async (options) => {
    ...
    const resolvePurchases = Platform.select({
        ...
        android: () => ExpoIapModule.getAvailableItems(normalizedOptions),
    }) ?? (() => Promise.resolve([]));
    ...
    if (Platform.OS === 'ios' || Platform.OS === 'android') {
        ...
        if (options.provider === 'iapkit' &&
            ...
            try {
                ...
                throw new Error('expo-constants is required for auto-filling iapkitApiKey from config. ' +
