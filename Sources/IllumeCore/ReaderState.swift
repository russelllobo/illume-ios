...
public enum IllumeLimits {
    ...
    public static let defaultUserStorageQuotaBytes = 104_857_600
}
...
public enum BillingAccess {
    ...
    public static func imageUsageCount(usage: ReaderImageUsage?, rowCount: Int, isPro: Bool) -> Int {
        guard let usage else { return rowCount }
        if isPro {
            return max(usage.monthlyGeneratedCount, rowCount)
        }
