...
final class IllumeAppModel: NSObject, ObservableObject {
    ...
    var imageUsageCount: Int {
        BillingAccess.imageUsageCount(usage: readerImageUsage, rowCount: 0, isPro: isPro)
    }
    ...
    func reload() async {
        ...
        await runBusy { [self] in
            ...
            billingProfile = try await profile
            readerImageUsage = try await usage
        }
    }
    ...
    func importDocument(from url: URL) async {
        ...
        await runImporting { [self] in
            ...
            guard storageUsed + data.count <= SupabaseConfig().storageQuotaBytes else {
    ...
    func generateImage(for paragraph: ReaderParagraph) async {
        ...
        await runBusy { [self] in
            readerImageResponse = try await backend.invokeReaderImage(
...
enum ReaderThemeChoice: String, CaseIterable {
    ...
    var background: Color {
        ...
        case .mint: Color(red: 0.92, green: 0.965, blue: 0.925)
        }
    }

    var foreground: Color {
        switch self {
        case .paper, .mint: IllumeTheme.ink
