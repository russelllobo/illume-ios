...
enum DocumentParser {
    ...
    private static func parseEpub(data: Data, fileName: String) throws -> ParsedDocument {
        ...
        let opfPath = capture(container, pattern: #"full-path="([^"]+)""#)
    ...
    private static func manifestItemPaths(opf: String, opfPath: String) -> [String] {
        ...
        let regex = try? NSRegularExpression(pattern: #"<item\b[^>]*href="([^"]+\.(?:xhtml|html|htm))"[^>]*media-type="application/xhtml\+xml"[^>]*/?>"#, options: [.caseInsensitive])
        let matches = regex?.matches(in: opf, range: NSRange(opf.startIndex..<opf.endIndex, in: opf)) ?? []
        return matches.compactMap { match in
            guard let range = Range(match.range(at: 1), in: opf) else { return nil }
            let href = String(opf[range])
            return base.isEmpty ? href : "\(base)/\(href)"
        }
    }
